    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,
        ...
        if mixer is not None:
            if self.args.mixer == "dmaq_qatten":
                ans_chosen, q_attend_regs, head_entropies, adv_w_final = \
                    mixer(chosen_action_qvals, batch["state"][:, :-1], is_v=True)
                ans_adv, _, _, adv_w_final = mixer(chosen_action_qvals, batch["state"][:, :-1], actions=actions_onehot,
            ...
            if self.args.double_q:
                if self.args.mixer == "dmaq_qatten":
                    ...
                    target_adv, _, _, adv_w_final = self.target_mixer(target_chosen_qvals, batch["state"][:, 1:],
